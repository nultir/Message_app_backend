...
class Create_conversation(ModelForm):
    Key = forms.CharField()
    Creator = forms.ChoiceField(queryset=User.objects.all())
    class Meta:
        model = Conversation
        fields = ["Key","Creator"]
...
class Add_to_conversation(ModelForm):
    User_id = forms.ChoiceField(queryset=User.objects.all())
    Conversation_id = forms.ModelChoiceField(queryset=Conversation.objects.all())
...
class Add_user_to_friends(ModelForm):
    User_1 = forms.ChoiceField(queryset=User.objects.all())
    User_2 = forms.ChoiceField(queryset=User.objects.all())
